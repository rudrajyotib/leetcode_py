...
from medium.tree import btree_right_view


class TreeRightViewTest(TestCase):

    def test_level_order_with_2_child_nodes(self):
        level1_child1 = btree_right_view.TreeNode(2, None, None)
        level1_child2 = btree_right_view.TreeNode(3, None, None)
        root_node = btree_right_view.TreeNode(1, level1_child1, level1_child2)

        s1 = btree_right_view.Solution()
        result = s1.rightSideView(root_node)
        self.assertEqual([1,3], result, "Traversal does not match")


    def test_level_order_with_0_child_nodes(self):
        root_node = btree_right_view.TreeNode(1)
    ...
    def test_level_order_with_mix_of_child_nodes(self):
        level1_child1 = btree_right_view.TreeNode(2, None, None)
        level1_child2 = btree_right_view.TreeNode(3, None, None)
        level2_child2 = btree_right_view.TreeNode(5, None, None)
        level1_child1.right = level2_child2
        root_node = btree_right_view.TreeNode(1, level1_child1, level1_child2)

        s1 = btree_right_view.Solution()
        result = s1.rightSideView(root_node)
        self.assertEqual([1,3,5], result, "Traversal does not match")


    def test_should_create_linked_list(self):
        linked_list = btree_right_view.LinkedList()
        linked_list.add(1)
        linked_list.add(2)
        linked_list.add(3)
        linked_list.add(4)
